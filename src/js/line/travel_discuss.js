            ...
            if(spotFiles.length<5) {
                if(data.result!=undefined&&data.result.code==200) {
                    $.each(data.result.data, function (index, file) {
                        ...
                        $("#upImages").append("<li data-id=''><img src='"+imgSrc+"' alt=''></li>")
...
function savePublish(){
    ...
    var id = OperationURL.getQueryString("productId");
    var imgs =getImages();
    var param = {
    ...
    saveTravelDiscuss(param,true,function(result){
        console.log(result);
        if(result.code==0){
            var new_url = OperationURL.getUrlParent(window.location.href) + "travel_comments.html?supplierId="+travel.travelId+"productId="+id;
...
//    var title = "用户点评";
//    var url = $(obj).attr("data-url")+"?supplierId="+travel.travelId+"productId="+id;
