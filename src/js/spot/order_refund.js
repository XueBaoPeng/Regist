...
var initSpotProductRefund = function(){
    ...
    getSpotProductRefund(param,false,function(result){
        if(result.code==0){
...
var initSpotRefundReason = function(){
    //TODO 退票原因
    var param = {};
    getSpotRefundReason(param,false,function(result){
        if(result.code==0){
...
var refundClick=function(){
    ...
    saveSpotOrderRefund(param,false,function(result){
        if(result.code==0){
            var title = "订单列表";
            var new_url = OperationURL.getUrlOrigin()+"personcenter/order_order.html";
