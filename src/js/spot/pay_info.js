...
function loadProductAndOrder(){
    ...
    //产品订单信息
    getSpotProductAndOrder(param, false, function(result){
        if(result.code == 0){
            spot.totalTime = result.data.effectiveTime;
            spot.orderAmount = parseFloat(result.data.orderMoney);
            spot.payment = parseFloat(result.data.orderMoney);
...
function payClick(){
    ...
    saveSpotOrderPay(param,true,function(result){
        if(result.code==0){
            if(payType==""){
                OperationURL.loadURL(webviewInterface.winType.normal, webviewInterface.mode.push, webviewInterface.orientation.portrait, "支付成功", "pay_success.html?id="+id);
            }
        }else{
            if(payType==""){
                $.happytoAlert(result.msg);
